...
- 标准识别失败后，自动执行一次本地图像增强重试。
- 校验18位身份证号格式、出生日期和校验码。
- 按身份证号去重，并标记姓名冲突。
- 将识别结果和待人工复核记录导出为 `.xlsx`。
...
- Windows/Linux：使用本机 Tesseract，需要 `tesseract` 命令位于 `PATH`，并已安装 `chi_sim`、`eng` 和 `osd` 语言包。
- 项目不需要安装第三方 Python 包。
...
标准识别失败时，程序会自动进行一次灰度化、对比度增强和锐化重试。重试成功的状态为“成功（增强重试）”。

“待人工复核”工作表包含以下三列：

| 来源文件 | 失败原因 | 建议操作 |
| --- | --- | --- |
| 原图片相对路径 | 最终识别失败或姓名冲突原因 | 重新拍摄、人工录入或核对首次记录 |

普通重复记录不会进入待人工复核表，程序也不会复制或嵌入身份证图片。
